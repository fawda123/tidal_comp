...
tosamp <- select(sims_day, date, lnQ_sim, sim3) %>% 
  rename(
    res = sim3,
    flo = lnQ_sim
    ) %>% 
...
gams_val <- foreach(val = 1:nrow(grd)) %dopar% {   
  ...
  trnmod <- try({gam(res~te(dec_time, doy, flo, bs=c("tp","cc","tp")), data = trndat, knots=list(doy=c(1,366)))})
  if(inherits(trnmod, 'try-error')) next
...
toplo <- mutate(grd, 
  ...
  group_by(blck, unts, var, mper) %>% 
  summarise(
    mean_val = median(val, na.rm = TRUE), 
    max_val = quantile(val, 0.95), #max(val, na.rm = TRUE), 
    min_val = quantile(val, 0.05)#min(val, na.rm = TRUE)
  ) %>% 
  rename(
    `Block sizes` = blck
  )
...
pdf('C:/Users/mbeck/Desktop/GAM_validation.pdf', height = 4, width = 8, family = 'serif')
print(p)
dev.off()
...
#   
#   plot(sim3 ~ date, tmp$alldat, xlab = '', ylab = '')
#   mtext(titleval, side = 3, cex = 0.8)
#   points(sim3 ~ date, tmp$alldat[tmp$smps, ], col = 'red', pch = 19)
#   
# }
