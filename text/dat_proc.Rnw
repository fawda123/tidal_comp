...
pax_flow <- read.csv(paste0(root_path, '/inst/PatuxentDailyFlow_84to15.csv'), 
  header = TRUE, stringsAsFactors = FALSE)
pax_flow <- mutate(pax_flow, date = as.Date(date, format = '%m/%d/%Y'))
...
# repeat but for bottom chlorophyll
bchl_tmp <- filter(pax_data, 
  PARAMETER == 'CHLA' & LAYER == 'B'& PROBLEM == ''
  ) %>% 
  mutate(lnchla = log(AvgValue)) %>%
  mutate(date = as.Date(date, format = '%m/%d/%Y')) %>% 
  select(date, STATION, lnchla)
...
pax_data <- full_join(schl_tmp, sal_tmp, by = c('date', 'STATION')) %>% 
  left_join(pax_flow, by = 'date')

# merge surface and bottom chlorophyll
pax_chldata <- full_join(schl_tmp, bchl_tmp, by = c('date', 'STATION')) %>% 
  mutate(
    lnchla_s = lnchla.x,
    lnchla_b = lnchla.y
    ) %>% 
  select(-lnchla.x, -lnchla.y) %>% 
  gather('loc', 'lnval', 3, 4)

# save the data
save_path <- gsub('text$', 'data', getwd())
save(pax_data, file = paste0(save_path, '/pax_data.RData'))
save(pax_chldata, file = paste0(save_path, '/pax_chldata.RData'))
