...
This plot shows changes in chlorophyll concentrations by time.
<<eval = F>>=
devtools::load_all()
data(pax_data)
data(pax_meta)
...
# format data for lotting
# add yr, mo, summarize to reduce size
# add ptsz column
# add color column
pax_plo <- mutate(
    pax_data,
  ...
  summarize(lnchla = mean(lnchla, na.rm = T)) %>% 
  mutate(

    ptsz = rescale(lnchla, c(3, 12)),
    cols = cRamp(lnchla)
    ) %>% 
  unite_('date', c('mo', 'yr'), sep = '/') %>% 
  mutate(date = as.Date(paste0('1/', date), '%d/%m/%Y')) %>% 
  arrange(STATION, date) %>% 
  mutate(STATION = as.character(STATION)) %>% 
  left_join(., pax_meta, by = 'STATION')

# base plot
pbase <- ggplot(pax_plo, aes(x = date, y = lnchla, group = STATION)) +
  geom_line() + 
...
# make plots for each station
dts <- sort(unique(pax_plo$date))
pdf('figs/pax_trnds.pdf', height = 5, width = 10, family = 'serif')
for(dt in dts){

  # add yr, month columns
  pax_dt <- filter(pax_plo, date == dt)
  ...
  # add date line to map
  pbase_ln <- pbase + 
    geom_vline(
      xintercept = as.numeric(dt), 
      linetype = 'dashed', size = 1.2
      )
  
#   # for putting the date on the plot
#   dt_txt <- data.frame(
#     LONG = -76.45,
#     LAT = 38.7,
#     lab = paste(strftime(dt, '%m'), strftime(dt, '%Y'), sep = '-')
#     )
  ...
  pmap <- ggplot(pax_meta, aes(x = LONG, y = LAT)) + 
    ...
    # geom_text(data = dt_txt, aes(label = lab)) + 
    geom_point(data = pax_dt, colour = 'black', size = pax_dt$ptsz) +
    theme(axis.title = element_blank())
   
  #combine
  grid.arrange(
    pbase_ln, pmap, ncol =2, widths = c(1, 0.5)
    )
